...
把本文当作唯一执行规范。目标：以 **czsc 为缠论内核**，集成Mystery趋势分析理论 的数据中枢、Mystery 规则、板块指数、扫描与日报/Web，且业务不绑死 czsc 内部对象。
...
工程现状与实现细节见 DESIGN_DOCUMENT.md，本文不写设计方案、不排期。
...
4. **混合分关闭**（生产默认 `MYSTERY_CHAN_SCORE=0`，或结构关闭）时，综合分与 misteryanalyze 1.22.30 兼容（金标三只分差 ≤ 1）。`MYSTERY_CHAN_ENABLED` 只控制结构展示，默认开，不单独改变综合分。
5. 本地行情库为分析权威数据；在线源可替换、可降级，分析不得绑定单一商业 API。
6. 产品能力覆盖 misteryanalyze 已有的投研闭环：数据更新、规则分析、扫描分类、日报/报告、Web 查看；缠论以结构摘要和可选展示为限，**加分默认关**。
...
- `turn` 只允许：官方或 legacy 原值，或 `volume（股）/ float_shares × 100` 且 `0 < turn < 80`。空值禁止用 0 或假数填。无有效近 20 日均换手必须 `chip_low_unknown`，不得判 `chip_low=True`。
- 流通股本低频更新（初始化 / 周对账 / 公司行动），禁止每个交易日全市场打竞价刷股本。
- 禁止用当前股本回填快照日 `as_of` 之前的历史空 `turn`。
- 写 K 线后必须失效该票 `analysis_cache`；分析缓存不得改变计分语义。
...
- 改列、加表必须走 `mystery/store/migrations/*.sql`，由 `_init_db` 记账执行；禁止只改 `schema.sql` 或只在 `_init_db` 里手写 ALTER。

### 2.5 评分

- 两个独立开关，不得再写成一个：
  - `MYSTERY_CHAN_ENABLED` / `chan.enabled`：结构展示（图、摘要），生产默认开。
  - `MYSTERY_CHAN_SCORE` / `chan.score`：是否把缠论分纳入综合分，生产默认关。
- 混合分关：综合分 = Mystery 1.22.30，不得改权重。
- 混合分开且结构开：允许 `S = 0.55×Mystery + 0.25×共振 + 0.20×缠论`；年线滤网未通过时混合分必须为 0。
- 只开结构、不开混合分：金标路径与关结构相同。
...
- `scan_jobs` / `scan_results` 的固定写手是 `czsc-mi scan`（含 18:00 cron）。同类型只保留最新一份。
- Web 默认只读已有 job。页面触发的扫描与 cron 必须互斥（flock）；冲突时拒绝写入，不排队双写。
...
- 仅增加展示字段、缓存、源适配、报表排版、换手派生与覆盖率观测、股本低频刷新，且不改变 `AnalysisResult` 计分语义的，不算规范变更。
