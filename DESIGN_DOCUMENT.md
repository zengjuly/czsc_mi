...
| W46 | **DuckDB 预同步双代码格式孤儿行修复（v0.10.20）**：`upsert_kline_many` 旧版 `code_col='thscode'` 路径裸 `str(code)` 写库（W12b 起潜伏），DuckDB 增量同步把 `600000.SH` 后缀原样落库，与读端 `sh.600000` 点格式错位——`kline_last_date` 永远看不到后缀行 → 2317 只 .SH + 2898 只 .SZ 每天被重拉重插成孤儿副本（点格式行停在 09-11，分析读到旧数据）。修复三层：① 写端新增 `_dbcode()`（复用 codes.db_code_of，非法码宽容回退原样小写，与 `_dot` 对称语义）接入 `upsert_kline_many`；② `scripts/merge_suffix_codes.py` 存量合并（幂等；冲突行以 suffix 更新鲜价格覆盖 + turn/pctChg COALESCE、无冲突改码迁入、先备份独立库再合并再删；实跑 25972 行并入，验收 后缀残留=0 重复=0）；③ merge 使超限票回潮后重跑 `_prune_daily` 同款重剪（1296+1911 票）并失效受影响 5215 票两套缓存（symbol=点后大写格式）。教训：**存量修复首轮探测 `GROUP BY code ORDER BY COUNT DESC LIMIT` 只见到 .SH 漏了 .SZ——按格式清理前必须先 `GLOB` 全后缀模式点验数量，勿以首屏样本定范围**。回归 226 passed（+3 `tests/test_w46_code_format_unify.py`）。**v0.10.21 防回潮**：`data_fingerprint` 新增 `suffix_orphans` 哨兵（`GLOB '*.[A-Z][A-Z]'`，点格式小写不误报），两处 QA 指纹行展示 + 非 0 时 WARN 查 `_dbcode` 归一/外部直写；全库 16 表格式扫描甄别：float_share_snapshot.thscode / scan_results.symbol 本为后缀设计列（读写一致），sector `.TI` 为已知混合约定，均非污染。 |
| W47 | **外部 review 逐条核实修复（v0.10.22）**：对第二份全仓 review 的 16 条先对码甄别再修。**采纳 4**：① `_avg_turnover_20` 加有效根数门槛（<MYSTERY_TURNOVER_20_MIN_VALID 默认 15 → None 走 chip_low_unknown，防 W43 洗假 0 后残值均值误打低换手）；② `_result_from_payload` 只取 MysteryBreakdown 已知字段（未知键不再 TypeError 致整池静默重算）；③ 管线降级报告醒目标注（cli 页脚+飞书：任一步 fail → 「⚠️DEGRADED 管线降级」，全部 ok 也明示）；④ `combine` 参数 chan_enabled→**mix_enabled** 改名（生产默认结构开/混合分关，旧名易误导传错致金标分变混合分）+ chan_score 背驰改精确匹配。**驳回 3（有码为证）**：量比含当日非漂移（金标旧仓 momentum_indicators.py:25 同式，注释钉死勿"修正"）；fork 继承 DB 连接不成立（MysteryDB 每次 _connect() 短连接，无常驻 self._conn）；sector .TI/thscode/symbol 后缀列为设计格式。挂账（P1/P2 未动）：Web ?at= 不绑 user/nonce、400 段无前缀码猜 BJ、ME/W resample 非交易日历、volume 量纲未入模型、主力资金文案、docs/006-008 旧计划口吻。回归 231 passed（+5 `tests/test_w47_review_fixes.py`，W35/W41 小样本测试显式降门槛保语义）。 |
| W48 | **W47 挂账消化 + 收尾（v0.10.23）**：① Web `?at=` token 加固为三代格式 `<exp>.<nonce>.<sig>`，签名负载绑 用户名+nonce（改口令→hash 变→旧 token 全量吊销；旧两段格式 verify 直接拒收），默认时效 7 天→**24h**；失败限速从 session_state 改**文件级** `~/.config/czsc_mi/web_auth_fails.json`（chmod 600，坏文件视为干净、写盘失败不阻塞登录），换标签页重置限速的洞闭合；用户名比较 `==` 改 `hmac.compare_digest`。② `normalize_symbol` 对无前缀 400/410/420（老三板）**raise ValueError 拒猜**（带显式前缀仍前缀优先；生产库该段存量=0 已核实）+ test_codes 补拒猜/BJ 号段断言。③ docs/006-008 加「已执行完毕、历史存档」状态头，消除旧计划口吻。④ CI 矩阵补 **3.12**（本机 venv 即 3.12，原先两版本从未覆盖生产解释器）。**新驳回（有码为证）**：review P1#8「周线 W 非交易日历」不成立——`market._RULE` 本就是 `W-FRI`/`ME`，与金标旧仓 kline_resampler 同规则（含 use_trading_calendar 可选开关），改日历对齐=单方面打偏周/月结构。仍未动挂账：volume 手/股量纲入 Bar 模型（展示层无消费方，改动波及指纹/缓存语义，收益零）、「主力资金连续流入」文案重命名（checklist 键为金标契约字面值，改名会打偏 8 项计数展示映射，维持键名+详情行「近3日N日上涨」已足够诚实）、`suffix_orphans>0` cron 硬失败（当前 QA WARN 已可见，硬失败需先观察一周误报率）。回归 233 passed（231 基线 + 新增 test_normalize_bj_and_laoban_refuse、test_fail_state_file_roundtrip）。 |
