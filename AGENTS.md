...
- 0.10.0 起（6C，010.md 30 只快照标定）：混合分路径的 `S_chan` 用标定规则表（010.md §9）；该路径 `rule_ver = mystery-0.10.0-chan`（缓存键含之，与分关旧缓存互不污染）；分关路径仍 `mystery-1.22.30-compat`、综合分 = Mystery 原值。
- 买卖点/背驰标签仅由 `CzscAdapter.signal_flags` 在混合分路径生成（分关零开销），core 仍不 import czsc。
